...
| `result_cache_hit` | **int** | Details whether a completed query used result set cache.<br /><br />**Applies to**: Azure Synapse Analytics | 1 = Result set cache hit<br />0 = Result set cache miss<br />`NULL` = Query was not a SELECT query.<br />Negative integer values = Reasons why result set caching wasn't used. See remarks section for details. |
